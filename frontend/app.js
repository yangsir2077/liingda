...
const TableDetail = {
          ...
          <table style="width:100%;border-collapse:collapse;min-width:500px">
            <thead>
              <tr>
                ...
                <th v-for="f in table.fields" :key="f.name" :style="hiddenCols.has(f.name) ? 'display:none' : ''" style="padding:8px 16px 4px;text-align:left;font-weight:700;font-size:13px;color:var(--text-secondary);border-bottom:1px solid var(--border);white-space:nowrap;background:var(--bg);user-select:none" :style="sortField===f.name ? 'color:var(--primary)' : ''">
  ...
  setup(props) {
    ...
    async function batchDelete() { if (!confirm(`确认删除选中的 ${selectedIds.value.size} 条记录？`)) return; try { await api.post(`/tables/${props.tableId}/records/batch`, { action: 'delete', ids: [...selectedIds.value] }); showToast(`已删除 ${selectedIds.value.size} 条`, 'success'); clearSelection(); loadRecords(); } catch (e) { showToast('批量删除失败', 'error'); } }
...
const App = {
  setup() {
    ...
    function confirmDialog(msg) { return new Promise(resolve => { showConfirm.value = { msg, ok: () => { resolve(true); showConfirm.value = null; }, cancel: () => { resolve(false); showConfirm.value = null; } }; }); }
    ...
    return { view: route, currentView, routeParams, navigateTo, logout, logoutWithConfirm, currentUser, isAdmin, showConfirm, confirmDialog };
  },
  template: `
